...
export interface AdminSchedulesViewProps {
  showHeader?: boolean;
}
...
export function AdminSchedulesView({
  ...
  return (
    <div className="space-y-6">
      {/* Top Header if showHeader is true */}
      {showHeader && (
        <div className="flex flex-col gap-4 border-b border-neutral-200 pb-5 sm:flex-row sm:items-center sm:justify-between dark:border-zinc-800">
          <div>
            ...
            <p className="mt-1 text-xs text-neutral-500 dark:text-zinc-400">
              Publish, update status, and manage attendance for club sharing events.
            </p>
          </div>
      ...
      {/* Action bar if header is hidden */}
      {!showHeader && (
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs font-semibold text-neutral-600 dark:text-zinc-400">
            <Calendar className="size-4 text-neutral-500" />
            <span>Tổng cộng: {schedules.length} buổi chia sẻ</span>
          </div>
          <Button onClick={handleCreate} size="sm" variant="brand">
