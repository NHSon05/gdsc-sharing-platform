...
- **Đã hoàn thành**: 24 / 24 (100%)
- **Trạng thái**: 🟢 Sprint 1 hoàn thành 100% (Ready for Production / Milestone Sign-off)
...
- [x] **TASK-101**: Bổ sung Policy names & Claim constants vào `GdscSharingPlatform.Application/Common/Security/RoleNames.cs` (hoặc tạo `AuthConstants.cs`).
  - _Output_: Khai báo các hằng số Policy (`AdminOnly`, `MemberOnly`, `RequireActiveUser`) và Custom Claims (`department_id`, `status`).
  - _FR Mapping_: FR-030, FR-034.
- [x] **TASK-102**: Tạo entity `RefreshToken` trong `GdscSharingPlatform.Infrastructure/Identity/RefreshToken.cs`.
  - _Fields_: `Id`, `UserId`, `TokenHash`, `CreatedAt`, `ExpiresAt`, `IsRevoked`, `RevokedAt`, `ReplacedByTokenHash`, `RevocationReason`, `CreatedByIp`, `UserAgent`.
  - _FR Mapping_: FR-009, FR-010, FR-011, SR-003.
...
- [x] **TASK-104**: Cấu hình Fluent API `RefreshTokenConfiguration` trong `GdscSharingPlatform.Infrastructure/Persistence/Configurations/RefreshTokenConfiguration.cs`.
  - _Rules_: Table `RefreshTokens`, PK `Id`, Unique Index trên `TokenHash`, Index trên `(UserId, IsRevoked, ExpiresAt)`, FK với `ApplicationUser` (Cascade delete).
  - _FR Mapping_: FR-009, FR-012.
...
- [x] **TASK-106**: Tạo EF Core Migration `AddRefreshTokenTable` và chạy migration trên database test/dev.
  - _Validation_: Đảm bảo không làm mất dữ liệu tài khoản Admin và Department đã seed ở Sprint 0.
  - _FR Mapping_: FR-043, FR-044, SC-017, SC-019.
...
- [x] **TASK-201**: Tạo model `JwtOptions.cs` trong `GdscSharingPlatform.Infrastructure/Identity/Options/JwtOptions.cs` và cấu hình `ValidateOnStart()` trong `DependencyInjection.cs`.
  - _Options_: `Issuer`, `Audience`, `SecretKey` (min 32 ký tự), `AccessTokenExpirationMinutes` (15m), `RefreshTokenExpirationDays` (7d), `ClockSkewSeconds` (0s).
  - _FR Mapping_: FR-042, SR-001, SR-004.
- [x] **TASK-202**: Khai báo interface `IJwtTokenGenerator` trong `GdscSharingPlatform.Application/Common/Interfaces/IJwtTokenGenerator.cs`.
  - _Methods_:
  ...
  - _FR Mapping_: FR-007, FR-008, FR-009, SR-002, SR-003, SR-005.
- [x] **TASK-203**: Triển khai `JwtTokenGenerator` trong `GdscSharingPlatform.Infrastructure/Identity/Services/JwtTokenGenerator.cs`.
  - _Claims_: `sub`, `email`, `name`, `role` (multiple), `department_id`, `status`, `jti`, `iss`, `aud`, `exp`, `nbf` (tính theo UTC).
  - _FR Mapping_: FR-027, FR-049, SR-005, SR-006, SR-007.
- [x] **TASK-204**: Khai báo interface `ICurrentUserService` trong `Application/Common/Interfaces/ICurrentUserService.cs` và triển khai `CurrentUserService` trong `Infrastructure`.
  - _Properties_: `UserId`, `Email`, `Roles`, `IsAuthenticated`.
  - _FR Mapping_: FR-026, FR-029.
...
- [x] **TASK-301**: Định nghĩa các DTOs trong `GdscSharingPlatform.Application/Features/Auth/Models/`:
  ...
  - _FR Mapping_: FR-027, FR-028, FR-038.
- [x] **TASK-302**: Viết FluentValidation Validators:
  ...
  - _FR Mapping_: FR-002, FR-003, FR-038, FR-046.
- [x] **TASK-303**: Khai báo interface `IAuthService` trong `GdscSharingPlatform.Application/Features/Auth/Interfaces/IAuthService.cs`.
  - _Methods_:
...
- [x] **TASK-304**: Triển khai `AuthService.LoginAsync` trong `GdscSharingPlatform.Infrastructure/Identity/Services/AuthService.cs`:
  ...
  - _FR Mapping_: FR-001 -> FR-006, FR-040, SR-008, SC-001, SC-002, SC-003.
- [x] **TASK-305**: Triển khai `AuthService.RefreshTokenAsync` (Rotation & Reuse Detection):
  ...
  - _FR Mapping_: FR-013 -> FR-019, FR-047, FR-048, SR-011, SR-012, SC-007, SC-008, SC-009.
- [x] **TASK-306**: Triển khai `AuthService.LogoutAsync` & `LogoutAllAsync`:
  ...
  - _FR Mapping_: FR-020 -> FR-025, SC-010, SC-011, SC-012.
- [x] **TASK-307**: Triển khai `AuthService.GetCurrentUserAsync`:
  - Nạp thông tin User kèm `Department` và `Roles`.
  - Trả về `CurrentUserDto` không chứa thông tin bảo mật nhạy cảm.
  - _FR Mapping_: FR-026, FR-027, FR-028, SC-013.
...
- [x] **TASK-401**: Cấu hình JWT Bearer Authentication & Authorization Policies trong `Program.cs` / `AddApiAuthentication`.
  - _TokenValidationParameters_: ValidateIssuer, ValidateAudience, ValidateLifetime, ValidateIssuerSigningKey, ClockSkew = TimeSpan.Zero.
  - _Policies_: `AdminOnly` (`Roles = "Admin"`), `MemberOnly` (`Roles = "Member,Admin"`).
  - _FR Mapping_: FR-030, FR-031, FR-032, FR-033, FR-036, FR-037.
- [x] **TASK-402**: Cấu hình Swagger / OpenAPI hỗ trợ Bearer Token Authentication (Authorize button trong Swagger UI).
  - _Config_: `AddSecurityDefinition("Bearer", ...)` & `AddSecurityRequirement(...)`.
  - _FR Mapping_: SC-020.
- [x] **TASK-403**: Tạo `AuthController` (`GdscSharingPlatform.Api/Controllers/AuthController.cs`):
  - `POST /api/auth/login` [AllowAnonymous]
  - `POST /api/auth/refresh` [AllowAnonymous]
  - `POST /api/auth/logout` [AllowAnonymous / Authorize]
  - `POST /api/auth/logout-all` [Authorize]
  - `GET /api/auth/me` [Authorize]
  - `GET /api/authorization-test/member` [Authorize(Policy = AuthPolicies.MemberOnly)]
  - `GET /api/authorization-test/admin` [Authorize(Policy = AuthPolicies.AdminOnly)]
  - _FR Mapping_: FR-001, FR-013, FR-020, FR-023, FR-026, FR-045.
- [x] **TASK-404**: Xác thực định dạng lỗi RFC 7807 và `traceId` với `GlobalExceptionHandler`.
  - Đảm bảo 401 Unauthorized và 403 Forbidden trả về format nhất quán có `traceId`.
  - _FR Mapping_: FR-034, FR-039, SR-010, SC-014.
...
- [x] **TASK-501**: Viết Unit Tests trong `GdscSharingPlatform.UnitTests`:
  ...
  - `CurrentUserServiceTests`: Kiểm tra trích xuất claims an toàn.
  - _FR Mapping_: FR-002, FR-003, FR-009, FR-038, SC-015, SC-016.
- [x] **TASK-502**: Viết Integration Tests trong `GdscSharingPlatform.IntegrationTests`:
  - `AuthEndpointsIntegrationTests`: Đăng nhập thành công với admin seed; Đăng nhập thất bại (sai email/password); Token rotation & reuse attack detection; Logout đơn phiên & all phiên; `/me` endpoint; Phân quyền `AdminOnly` vs `MemberOnly` (401, 403, 200).
  - _FR Mapping_: SC-001 -> SC-014, SC-016, SC-018.
- [x] **TASK-503**: Chạy Regression Testing cho toàn bộ test suite Sprint 0:
...
- [x] **TASK-601**: Kiểm tra đối chiếu bảng 20 tiêu chí nghiệm thu (**SC-001 -> SC-020**).
- [x] **TASK-602**: Kiểm tra OpenAPI Swagger UI hoạt động thông suốt (Authorize -> Gọi thử các endpoint).
- [x] **TASK-603**: Cập nhật tài liệu nghiệm thu Sprint 1 (`walkthrough.md` hoặc báo cáo nghiệm thu).
